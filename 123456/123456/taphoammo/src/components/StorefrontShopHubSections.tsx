...
function ShopHubSponsoredCarousel({
  items,
  onSponsoredClick,
}: {
...
function ShopHubCategoryDropdown() {
  ...
  const listId = useRef(`shop-hub-cat-${Math.random().toString(36).slice(2)}`).current;
...
function ShopHubSortDropdown() {
  ...
  const triggerRef = useRef<HTMLButtonElement>(null);
  const panelRef = useRef<HTMLDivElement>(null);
  const listId = useRef(`shop-hub-sort-${Math.random().toString(36).slice(2)}`).current;
  const [panelStyle, setPanelStyle] = useState<CSSProperties>({});
